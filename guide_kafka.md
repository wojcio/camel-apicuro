...
-   **Topic**: A category or feed name to which records are published (e.g., `user-events`).
-   **Broker**: A Kafka server that stores data and serves clients. Our setup uses a single broker.
-   **Producer**: An application that sends data to Kafka topics (e.g., our `produce-valid-event` route).
-   **Consumer**: An application that reads data from Kafka topics (e.g., our `consume-user-events` route).
...
## 3. Useful CLI Operations (via Podman)

Since Kafka is running in a container named `kafka`, you can use `podman exec` to run internal scripts:

### List Topics
```bash
podman exec -it kafka /opt/bitnami/kafka/bin/kafka-topics.sh --bootstrap-server localhost:9092 --list
```

### Describe a Topic
```bash
podman exec -it kafka /opt/bitnami/kafka/bin/kafka-topics.sh --bootstrap-server localhost:9092 --topic user-events --describe
```

### Consume Messages Manually (Console Consumer)
```bash
podman exec -it kafka /opt/bitnami/kafka/bin/kafka-console-consumer.sh --bootstrap-server localhost:9092 --topic user-events --from-beginning
```
...
- Ensure the `ADVERTISED_LISTENERS` in `compose.yaml` matches your host access (currently set to `localhost:9092`).
